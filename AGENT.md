...
**OAuth2 flow:** Connect endpoint generates HMAC-signed state stored in KV with 10-min expiry → redirects to OIDC provider → callback validates state signature/expiry → exchanges code for tokens → verifies ID token via JWKS → extracts claims using configurable claim mappings → creates/links/updates Mattermost user → **web/desktop:** sets MMAUTHTOKEN session cookie and redirects; **mobile** (state carries a valid `MobileRedirect`): renders an HTML auto-redirect to the app's custom URL scheme (`mmauth://callback?MMAUTHTOKEN=…&MMCSRF=…`) instead of the cookie.
