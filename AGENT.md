...
**OAuth2 flow:** Connect endpoint generates HMAC-signed state stored in KV with 10-min expiry → redirects to OIDC provider → callback validates state signature/expiry → exchanges code for tokens → verifies ID token via JWKS → extracts claims using configurable claim mappings → creates/links/updates Mattermost user → **web:** sets MMAUTHTOKEN session cookie and 302-redirects; **desktop popup** (state carries `Popup`, set by the webapp's `window.open` flow): sets the cookie then renders an HTML page that moves `window.opener` to the return path + closes the popup (needed because the Desktop app blocks main-window external navigation); **mobile** (state carries a valid `MobileRedirect`): renders an HTML auto-redirect to the app's custom URL scheme (`mmauth://callback?MMAUTHTOKEN=…&MMCSRF=…`) instead of the cookie.
